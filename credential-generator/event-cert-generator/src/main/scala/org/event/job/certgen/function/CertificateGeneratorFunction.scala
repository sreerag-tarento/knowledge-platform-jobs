package org.event.job.certgen.function
...
import org.event.job.certgen.task.EventCertificateGeneratorConfig
...
class CertificateGeneratorFunction  (config: EventCertificateGeneratorConfig, httpUtil: HttpUtil, storageService: StorageService, @transient var cassandraUtil: CassandraUtil = null)
  ...
  private def generateAuditEvent(data: UserEnrollmentData): CertificateAuditEvent = {
    CertificateAuditEvent(
      ...
      `object` = EventObject(id = data.certificate.id, `type` = "Certificate", rollup = Map(config.l1 -> data.eventId).asJava))
  }
