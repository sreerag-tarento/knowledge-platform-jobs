...
case class NotificationMetaData(userId: String, courseName: String, issuedOn: Date, courseId: String, templateId: String, partition: Int, offset: Long, courseProvider: String, coursePosterImage:String,username:String)
...
/*      } else {
  ...
  private def getUserDetails(userId: String)(metrics: Metrics): Map[String, AnyRef] = {
    ...
    if (200 == httpResponse.status) {
      logger.info("user read response status {}", httpResponse.status)
