...
class CommonCertificateGeneratorStreamTask(config: CertificateGeneratorConfig, kafkaConnector: FlinkKafkaConnector, httpUtil: HttpUtil, storageService: StorageService) {
  ...
  def process(): Unit = {
    ...
    val processStreamTask = env.addSource(source)
      ...
      .name("common-certificate-generator-utility")
      .uid("common-certificate-generator-utility")
...
object CommonCertificateGeneratorStreamTask {
  ...
  def main(args: Array[String]): Unit = {
    ...
    }.getOrElse(ConfigFactory.load("common-certificate-generator-utility.conf").withFallback(ConfigFactory.systemEnvironment()))
