package org.sunbird.job.useranalysis.function.spec
...
import org.sunbird.job.useranalysis.fixture.EventFixture
...
class UserActivityAnalysisUpdaterFnTestSpec extends BaseTestSpec {
    ...
    lazy val jobConfig: UserActivityAnalysisUpdaterConfig = new UserActivityAnalysisUpdaterConfig(config)
    ...
    "CertPreProcess " should " issue certificate to valid request" in {
        ...
        val certEvent = new UserActivityAnalysisUpdaterFn(jobConfig, mockHttpUtil)(stringTypeInfo, cassandraUtil).issueCertificate(event, template)(cassandraUtil, cache, contentCache, mockMetrics, jobConfig, mockHttpUtil)
    ...
    "CertPreProcess When User Last Name is Empty " should " Should get Valid getRecipientName " in {
        ...
        jedis.select(jobConfig.contentCacheStore)
        jedis.set("content_001", """{"identifier":"content_001","contentType": "selfAssess"}""")
        val certEvent: String = new UserActivityAnalysisUpdaterFn(jobConfig, mockHttpUtil)(stringTypeInfo, cassandraUtil).issueCertificate(event, template)(cassandraUtil, cache, contentCache, mockMetrics, jobConfig, mockHttpUtil)
        certEvent shouldNot be(null)
