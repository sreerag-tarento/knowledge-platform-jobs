...
import org.sunbird.job.useranalysis.task.{UserActivityAnalysisUpdaterConfig, UserActivityAnalysisUpdaterTask}
...
class CollectionCertPreProcessorTaskSpec extends BaseTestSpec {
    ...
    val jobConfig: UserActivityAnalysisUpdaterConfig = new UserActivityAnalysisUpdaterConfig(config)
    ...
    "CollectionCertPreProcessor " should "validate metrics " in {
        initialize()
        new UserActivityAnalysisUpdaterTask(jobConfig, mockKafkaUtil, mockHttpUtil).process()
