...
import org.sunbird.job.useractivity.domain.{AssessedUser, AssessmentUserAttempt, EnrolledUser, Event}
...
trait IssueCertificateHelper {
  ...
  def getAPICall(url: String, responseParam: String)(config: UserActivityAnalysisUpdaterConfig, httpUtil: HttpUtil, metrics: Metrics): Map[String, AnyRef] = {
