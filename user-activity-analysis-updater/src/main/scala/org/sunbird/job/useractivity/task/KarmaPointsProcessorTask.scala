...
//import java.io.File
//
//class KarmaPointsProcessorTask(config: UserActivityAnalysisUpdaterConfig, kafkaConnector: FlinkKafkaConnector, httpUtil: HttpUtil) {
...
//
//    env.addSource(kafkaConnector.kafkaJobRequestSource[Event]("user-dash-state-test"))
//      .name(config.certificatePreProcessorConsumer)
